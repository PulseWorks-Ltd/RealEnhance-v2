...
// sky with a generated blue one, and boost vegetation colour/vibrancy. The
// prompt (STAGE1A_SUNNY_EXTERIOR_INSTRUCTION_BLOCK, pipeline/stage1A.ts) is
// explicit that buildings, structures, civil elements, and vegetation
// shape/position must never change — but nothing in the pipeline actually
// checks whether the model complied. Every existing window/opening
...
// genuinely present before" — explicitly instructed to ignore the sky and
// vegetation-colour changes this feature is intentionally allowed to make
// (both confirmed as wanted behaviour, not something to restrict).
//
// SCOPE: only ever invoked when the sunny-exterior prompt block was actually
// injected for this job (checkbox on, interior scene, non-empty prompt) —
// see pipeline/stage1A.ts's stage1ASunnyExteriorPromptInjected. Every other
// Stage 1A job pays zero extra cost or latency for this check.
//
...
// — specifically watched for false positives against the permitted sky and
// vegetation-colour changes — before being trusted to alter job output.
...
The enhancement pass was EXPLICITLY AUTHORISED to improve the outdoor weather, sky, lighting, and foliage colour visible through any existing window, door, or other opening — this is expected and must never be flagged. It was NOT authorised to invent any new exterior object, building, structure, or landscape feature that was not already present in BEFORE.

Your only task: decide whether the AFTER photo shows any exterior content, visible through a window, door, skylight, or other opening, that was NOT genuinely present in BEFORE.
...
- Vegetation colour changes: grass, lawns, trees, shrubs, and gardens may look greener, healthier, and more vibrant in AFTER. A colour or vibrancy change alone is authorised.
...
- A tree, shrub, hedge, or plant that is newly present, or an existing one that has changed size, shape, position, or species. (A colour or vibrancy change alone is fine — see PERMITTED above.)
- Exterior content present in BEFORE that has been removed or painted out in AFTER.

### HOW TO DECIDE
- Judge only the existence and identity of exterior content — never its colour, brightness, or weather condition.
- If a difference can be fully explained by weather, sky, lighting, exposure, or colour alone, it is NOT a failure.
...
- Only report hallucinationDetected = true when you are genuinely confident that new, different, or removed structural or object content is present — not merely because the same scene now looks more vivid.

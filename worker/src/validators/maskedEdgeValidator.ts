/**
 * Masked Edge Geometry Validator for Stage-2
 *
 * Detects architectural structural changes using masked edge analysis while
 * ignoring furniture and décor. Runs in log-only mode; callers decide how to
 * interpret the signals.
 */

import sharp from "sharp";
import { StageId } from "./stageAwareConfig";

export interface MaskedEdgeResult {
  createdOpenings: number;
  closedOpenings: number;
  maskedEdgeDrift: number;
}

const MASKED_EDGE_DRIFT_THRESHOLD = 0.18;
const DOOR_CREATION_THRESHOLD = 0.25;
const WINDOW_CREATION_THRESHOLD = 0.30;

async function buildStructuralMask(imagePath: string): Promise<{ mask: Buffer; width: number; height: number }> {
  try {
    const { data, info } = await sharp(imagePath)
      .resize(512, 512, { fit: "inside" })
      .greyscale()
      .raw()
      .toBuffer({ resolveWithObject: true });

    const { width, height } = info;

    const edges = Buffer.alloc(width * height);
    const EDGE_THRESHOLD = 80;

    for (let y = 1; y < height - 1; y++) {
      for (let x = 1; x < width - 1; x++) {
        const idx = y * width + x;

        const gx =
          -1 * data[(y - 1) * width + (x - 1)] + 1 * data[(y - 1) * width + (x + 1)] +
          -2 * data[y * width + (x - 1)] + 2 * data[y * width + (x + 1)] +
          -1 * data[(y + 1) * width + (x - 1)] + 1 * data[(y + 1) * width + (x + 1)];

        const gy =
          -1 * data[(y - 1) * width + (x - 1)] - 2 * data[(y - 1) * width + x] - 1 * data[(y - 1) * width + (x + 1)] +
          1 * data[(y + 1) * width + (x - 1)] + 2 * data[(y + 1) * width + x] + 1 * data[(y + 1) * width + (x + 1)];

        const magnitude = Math.sqrt(gx * gx + gy * gy);
        edges[idx] = magnitude > EDGE_THRESHOLD ? 255 : 0;
      }
    }

    const mask = Buffer.alloc(width * height);
    const verticalThreshold = height * 0.3;
    const horizontalThreshold = width * 0.3;

    // Strong vertical lines (walls, doors)
    for (let x = 0; x < width; x++) {
      let verticalStrength = 0;
      for (let y = 0; y < height; y++) {
        if (edges[y * width + x] > 0) verticalStrength++;
      }
      if (verticalStrength > verticalThreshold) {
        for (let y = 0; y < height; y++) {
          if (edges[y * width + x] > 0) mask[y * width + x] = 255;
        }
      }
    }

    // Strong horizontal lines (windows, ceiling/floor transitions)
    for (let y = 0; y < height; y++) {
      let horizontalStrength = 0;
      for (let x = 0; x < width; x++) {
        if (edges[y * width + x] > 0) horizontalStrength++;
      }
      if (horizontalStrength > horizontalThreshold) {
        for (let x = 0; x < width; x++) {
          if (edges[y * width + x] > 0) mask[y * width + x] = 255;
        }
      }
    }

    return { mask, width, height };
  } catch (err) {
    console.warn("[maskedEdge] Structural mask creation error:", err);
    const { width, height } = await sharp(imagePath).metadata();
    return {
      mask: Buffer.alloc((width || 512) * (height || 512)),
      width: width || 512,
      height: height || 512,
    };
  }
}

function applyLowerMaskExclusion(mask: Buffer, width: number, height: number, excludeLowerPct: number): Buffer {
  const pct = Number.isFinite(excludeLowerPct) ? Math.min(Math.max(excludeLowerPct, 0), 0.95) : 0;
  if (pct <= 0) return Buffer.from(mask);

  const cutoffRow = Math.floor(height * (1 - pct));
  const safeCutoff = Math.max(0, Math.min(height, cutoffRow));
  const copy = Buffer.from(mask);

  for (let y = safeCutoff; y < height; y++) {
    const start = y * width;
    copy.fill(0, start, start + width);
  }

  return copy;
}

function computeMaskedEdgeDrift(originalMask: Buffer, enhancedMask: Buffer, width: number, height: number): number {
  try {
    let origCount = 0;
    let enhCount = 0;

    for (let i = 0; i < width * height; i++) {
      if (originalMask[i] > 0) origCount++;
      if (enhancedMask[i] > 0) enhCount++;
    }

    if (origCount === 0 && enhCount === 0) return 0;

    const maxCount = Math.max(origCount, enhCount, 1);
    return Math.abs(enhCount - origCount) / maxCount;
  } catch (err) {
    console.warn("[maskedEdge] Drift computation error:", err);
    return 0;
  }
}

function detectDirectionalOpeningChange(
  originalMask: Buffer,
  enhancedMask: Buffer,
  width: number,
  height: number
): { verticalDelta: number; horizontalDelta: number } {
  try {
    let origVertical = 0;
    let enhVertical = 0;
    let origHorizontal = 0;
    let enhHorizontal = 0;

    for (let x = 0; x < width; x++) {
      let origVertStrength = 0;
      let enhVertStrength = 0;

      for (let y = 0; y < height; y++) {
        const idx = y * width + x;
        if (originalMask[idx] > 0) origVertStrength++;
        if (enhancedMask[idx] > 0) enhVertStrength++;
      }

      if (origVertStrength > height * 0.2) origVertical++;
      if (enhVertStrength > height * 0.2) enhVertical++;
    }

    for (let y = 0; y < height; y++) {
      let origHorizStrength = 0;
      let enhHorizStrength = 0;

      for (let x = 0; x < width; x++) {
        const idx = y * width + x;
        if (originalMask[idx] > 0) origHorizStrength++;
        if (enhancedMask[idx] > 0) enhHorizStrength++;
      }

      if (origHorizStrength > width * 0.2) origHorizontal++;
      if (enhHorizStrength > width * 0.2) enhHorizontal++;
    }

    const verticalDelta = (enhVertical - origVertical) / Math.max(origVertical, 1);
    const horizontalDelta = (enhHorizontal - origHorizontal) / Math.max(origHorizontal, 1);

    return { verticalDelta, horizontalDelta };
  } catch (err) {
    console.warn("[maskedEdge] Directional detection error:", err);
    return { verticalDelta: 0, horizontalDelta: 0 };
  }
}

export async function runMaskedEdgeValidator({
  originalImagePath,
  enhancedImagePath,
  stage,
  scene,
  mode = "log",
}: {
  originalImagePath: string;
  enhancedImagePath: string;
  stage: StageId;
  scene: "interior" | "exterior";
  mode?: "log";
}): Promise<MaskedEdgeResult> {
  console.log(`[MASK][stage2] Starting masked edge geometry validation (${scene})`);
  void mode; // mode is log-only today but kept for API compatibility

  try {
    const [originalStructure, enhancedStructure] = await Promise.all([
      buildStructuralMask(originalImagePath),
      buildStructuralMask(enhancedImagePath),
    ]);

    if (originalStructure.width !== enhancedStructure.width || originalStructure.height !== enhancedStructure.height) {
      console.warn(
        `[MASK][stage2] Dimension mismatch: ${originalStructure.width}x${originalStructure.height} vs ${enhancedStructure.width}x${enhancedStructure.height}`
      );
      return { createdOpenings: 0, closedOpenings: 0, maskedEdgeDrift: 0 };
    }

    const { width, height } = originalStructure;
    const applyArchitectureMask = stage === "stage2" && scene === "interior";
    const excludeLowerPct = applyArchitectureMask
      ? Number(process.env.STRUCT_VALIDATION_STAGE2_EXCLUDE_LOWER_PCT ?? 0.20)
      : 0;

    const originalMask = applyArchitectureMask
      ? applyLowerMaskExclusion(originalStructure.mask, width, height, excludeLowerPct)
      : originalStructure.mask;
    const enhancedMask = applyArchitectureMask
      ? applyLowerMaskExclusion(enhancedStructure.mask, width, height, excludeLowerPct)
      : enhancedStructure.mask;

    if (applyArchitectureMask) {
      console.log(
        `[MASK][stage2] Applied architecture-only mask (excluded ${(excludeLowerPct * 100).toFixed(0)}% lower region)`
      );
    }

    const maskedDrift = computeMaskedEdgeDrift(originalMask, enhancedMask, width, height);
    const directionChange = detectDirectionalOpeningChange(originalMask, enhancedMask, width, height);

    let createdOpenings = 0;
    let closedOpenings = 0;

    if (maskedDrift > MASKED_EDGE_DRIFT_THRESHOLD) {
      if (directionChange.verticalDelta > DOOR_CREATION_THRESHOLD && directionChange.verticalDelta > Math.abs(directionChange.horizontalDelta)) {
        createdOpenings = 1;
      } else if (directionChange.horizontalDelta > WINDOW_CREATION_THRESHOLD) {
        createdOpenings = 1;
      }

      if (directionChange.verticalDelta < -DOOR_CREATION_THRESHOLD) {
        closedOpenings = 1;
      } else if (directionChange.horizontalDelta < -WINDOW_CREATION_THRESHOLD) {
        closedOpenings = 1;
      }
    }

    const result: MaskedEdgeResult = {
      createdOpenings,
      closedOpenings,
      maskedEdgeDrift: maskedDrift,
    };

    console.log(
      `[MASK][stage2] masked drift: ${(maskedDrift * 100).toFixed(2)}% ` +
        (maskedDrift > MASKED_EDGE_DRIFT_THRESHOLD ? "(FAIL)" : "(OK)")
    );

    console.log(
      `[MASK][stage2] openings created: +${createdOpenings}, closed: -${closedOpenings} ` +
        (createdOpenings || closedOpenings ? "(FAIL)" : "(OK)")
    );

    console.log(
      `[MASK][stage2] RESULT: ${
        createdOpenings || closedOpenings || maskedDrift > MASKED_EDGE_DRIFT_THRESHOLD ? "FAILED" : "PASSED"
      } (log-only)`
    );

    return result;
  } catch (err) {
    console.error("[MASK][stage2] Masked edge validation error (non-fatal):", err);
    return { createdOpenings: 0, closedOpenings: 0, maskedEdgeDrift: 0 };
  }
}

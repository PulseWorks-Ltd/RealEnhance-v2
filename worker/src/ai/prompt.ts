...
export function buildQualityOnlyPrompt(opts: PromptOptions): string {
  ...
  if (activeScene === "exterior") {
    lines.push(
      ...
      "",
      "SURFACE CLEANING (GENTLE 30% REDUCTION APPROACH):",
      "• DRIVEWAYS/CONCRETE: REDUCE (not eliminate) oil stains, tire marks, and weathering by ~30% only. Show maintained but realistic condition - never make surfaces look freshly pressure-washed or brand new.",
      "• DECKING: Spot clean timber only where visibly soiled - fully preserve natural aging, weathering, and patina. REDUCE stains by ~30% maximum.",
      "• PATHS/PATIOS: Gentle brightening only (~30% stain reduction). Preserve realistic wear and aging - avoid 'freshly cleaned' appearance.",
      "• BUILDING EXTERIOR: Gentle brightening of paint/siding. Reduce mold/weathering stains by ~30% only. Preserve realistic maintenance level.",

...
const STRUCTURAL_INVARIANT_MODEL = String(process.env.GROK_VISION_MODEL || "grok-4.5");
...
plus one short explanation sentence.`;
  ...
  const rawText = (await grokAnalyzeImages({
    images: [
    ...
    reason: "structural_topology_check",
    expectJson: false,
  })).trim();
...
async function runStage1BFullFurnitureGeminiValidation(opts: {
  ...
  const model = opts.modelOverride || process.env.GROK_VISION_MODEL || "grok-4.5";
  ...
  try {
    const rawText = (await grokAnalyzeImages({
      images: [
        { buffer: Buffer.from(stage1A, "base64"), mimeType: "image/webp", label: "IMAGE_STAGE1A_BASELINE:" },
        { buffer: Buffer.from(stage1B, "base64"), mimeType: "image/webp", label: "IMAGE_STAGE1B_RESULT:" },
      ],
      prompt: validationPrompt,
      jobId: opts.jobId,
      imageId: opts.imageId,
      reason: "stage1b_full_furniture_validation",
    })).trim();
...
async function runStructuralInvariantGeminiCheck(
  ...
  const rawText = (await grokAnalyzeImages({
    ...
    reason: "structural_invariant_check",
  })).trim();
...
async function handleEnhanceJob(payload: EnhanceJobPayload) {
  ...
  const USE_FROM_EMPTY_VALIDATION_FOR_REFRESH = true;
  ...
  const normalizeForensicDecisionSource = (blockedBy?: string | null): "local" | "gemini" | "consensus" | "specialist" => {
  ...
  const stage2BlockedByPriority = (blockedBy?: string | null): number => {
    ...
    if (blockedBy === "specialist") return 3;
        ...
        runCompliance: async () => {
          ...
          if (!stage2OnlyBlockedReason && geminiSemanticValidatorMode !== null) {
            try {
              ...
              const s2oCompliance = await checkCompliance(baseRef.data, enhanced.data, {
                validationMode: stage2OnlySelectedValidationMode,
                jobId: payload.jobId,
                imageId: payload.imageId,
                attempt: stage2OnlyAttemptNo,
  ...
  try {
    ...
    if (sceneLabel === "exterior" && outdoorStagingEnabled) {
      try {
        ...
        const stagingResult = await detectStagingArea(base64);
        ...
        if (allowStaging) {
          stagingRegion = await detectStagingRegion(base64);
  ...
  if (payload.options.virtualStage) {
    ...
    const logRefreshValidationTrace = (params: {
      specialistHardFail: boolean;
      geminiDecision: "PASS" | "FAIL";
      finalDecision: "ACCEPT" | "RETRY";
      reason?: string;
    }): void => {
      if (!isRefreshValidationFlow) return;
      nLog("[STAGE2_REFRESH_TRACE]", {
        jobId: payload.jobId,
        mode: "REFRESH",
        localUsed: false,
        specialistHardFail: params.specialistHardFail,
        geminiDecision: params.geminiDecision,
...
All openings must remain identical in position and size to the original image.`;
      ...
      effectiveValidationMode = "enforce";
      const stage2GeminiPolicy: "always" = "always";
      nLog(`[worker] Unified validation mode: configured=${structureValidatorMode} effective=${effectiveValidationMode} blocking=ON (deterministic_stage2)`);
      ...
      type Stage2SignalValidator = "openings" | "fixtures" | "floor" | "envelope";
        ...
        runCompliance: async ({ unifiedValidation: complianceUnifiedValidation }) => {
          ...
          if (validationStage === "2") {
            try {
              ...
              nLog("[GROK_ESCALATION]", {
                jobId: payload.jobId,
                imageId: payload.imageId,
                attempt,
                action: "run",
                model: process.env.GROK_VISION_MODEL || "grok-4.5",
              ...
              const complianceBlockStartedAt = Date.now();
              compliance = await checkCompliance(base1A.data, baseFinal.data, {
                validationMode: stage2SelectedValidationMode,
                advisorySignals: complianceAdvisorySignals,

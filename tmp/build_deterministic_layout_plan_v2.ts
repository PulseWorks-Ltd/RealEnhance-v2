...
async function main() {
  ...
  if (resultsFiles.length === 0) {
    console.error("No wall_visibility_v2_results_*.json found — run investigate_wall_visibility_v2.ts first.");
    process.exit(1);
  }
  ...
  for (const runKey of ["run1", "run2"] as const) {
    ...
    console.log(`\n=== DETERMINISTIC PLAN using CORRECTED (v2) ${runKey} wall-visibility data (wallCount=${raw[runKey].parsed.wallCount}) ===`);
    console.log(JSON.stringify(plan, null, 2));
  }
}

main().catch((e) => {
  console.error("build_deterministic_layout_plan_v2 failed:", e);
  process.exit(1);
});

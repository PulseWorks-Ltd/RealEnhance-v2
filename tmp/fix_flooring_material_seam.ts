...
// ── PART 1: category-A locks, with the new flooring-material-boundary
// clause added to the existing Floors & Ceilings section (same status —
// general, unconditional, room-type-independent — as the AC-unit/
// ceiling-fixture/light-fixture clauses added earlier tonight). ──
...
Flooring material boundaries: Do not alter, blend, replace, or unify the flooring material anywhere in the room. If two or more distinct flooring materials are visible in the original photo (e.g. carpet, linoleum, hardwood, tile), each material must remain in its original location, boundary, and type, exactly as shown in the original photo. Do not extend one flooring type over another, and do not smooth over or remove a visible flooring transition/seam.
...
async function generateOne(imgName: string, baseline: any, prompt: string) {
  ...
  const outPath = path.join(LIVING_DIR, imgName.replace(/\.jpg$/i, "-staged-v3.webp"));
...
main().catch((e) => {
  console.error("fix_flooring_material_seam failed:", e);
  process.exit(1);
});

...
async function main() {
  ...
  // Temperature sweep: 0.40 (production default, already failed 3x with
  // varying prompt text — not re-run here) vs. two meaningfully lower
  // values. 0.15 tests "less random but not fully greedy"; 0.0 tests the
  // deterministic/greedy floor. If flooring compliance depends on sampling
  // randomness rather than prompt wording, the effect should show up
  // somewhere in this range.
  const temperatures = [0.15, 0.0];
  for (const temp of temperatures) {
    const suffix = `temp${String(temp).replace(".", "")}`;
    await generateOne("Living 07.jpg", living07Prompt, temp, suffix);
  }
...
main().catch((e) => {
  console.error("test_flooring_temperature failed:", e);
  process.exit(1);
});
